...
// Borrowed heavily from github.com:aelsabbahy/goss/resource/validate.go
...
func compileGomegaMatcher(matcher interface{}) (types.GomegaMatcher, error) {
	...
	case "have-key":
		subMatcher, err := compileGomegaMatcher(value)
		if err != nil {
			return nil, err
		}
...
func mapToGomega(value interface{}) (subMatchers []types.GomegaMatcher, err error) {
	...
	for _, key := range keys {
		val := valueI[key]
		val, err = compileGomegaMatcher(val)
		if err != nil {
			return
		}
...
func sliceToGomega(value interface{}) ([]types.GomegaMatcher, error) {
	...
	for _, v := range valueI {
		subMatcher, err := compileGomegaMatcher(v)
		if err != nil {
			return nil, err
		}
		subMatchers = append(subMatchers, subMatcher)
